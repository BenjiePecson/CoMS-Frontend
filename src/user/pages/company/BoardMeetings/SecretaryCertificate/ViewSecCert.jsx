...
const ViewSecCert = () => {
  const { companyId, seccert_id } = useParams();
  const selectedCompany = useSelector((state) => state.company.selectedCompany);

  const selectedSecretaryCertificate = useSelector(
    (state) => state.secretaryCertificate.selectedSecretaryCertificate
  );
          ...
          <div className="modal-box w-11/12 max-w-5xl p-2 rounded-sm">
            ...
            <div className="pt-5 flex flex-row justify-end">
              <button
                ...
                onClick={() => {
                  setEditFolderID(selectedSecretaryCertificate.gdrivefolder_id);
                  document.getElementById("setSecCertCompleted").showModal();
                }}
              >
                Mark as Completed
              </button>
              ...
              <button
                ...
                onClick={async (e) => {
                  ...
                  try {
                    ...
                    document.getElementById("setSecCertCompleted").close();
                  }
                }}
              >
                Submit
  ...
  useEffect(() => {
    dispatch(fetchSecretaryCertificate({ companyId, seccert_id }));
  }, []);

  useEffect(() => {
    setFormData(selectedSecretaryCertificate.details);
    setPreviewData(selectedSecretaryCertificate.details);
    setSelectedSecCertType(selectedSecretaryCertificate.type);
  }, [selectedSecretaryCertificate]);
      ...
      <div className="flex flex-col flex-1">
        ...
        <div className="flex flex-col md:flex-row gap-2 py-5 items-start flex-1">
          <div className="flex flex-col w-full md:w-[40%] bg-white p-5 rounded-lg gap-5 shadow-lg">
            ...
            <h1 className="font-bold">{selectedSecretaryCertificate.type}</h1>
            {/* <div>{preEmptiveRightsForm()}</div> */}
            <div>{getForm(selectedSecCertType)}</div>

            {selectedSecretaryCertificate.status != "Completed" && (
              <div className="">
                <button
